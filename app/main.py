...
from app.models import Audit, Job, Offer, Order, OrderLine, Outbox, Product, utcnow
from app.schemas import AuditDecision, CustomerReply, OrderInput
from app.services import audit, aware, lines_for

app = FastAPI(title="OrderOps AI — local learning MVP", version="0.1.0")
api_key = APIKeyHeader(name="X-API-Key", auto_error=False)


def admin(key: str | None = Depends(api_key)):
...
@app.post("/orders", status_code=202, dependencies=[Depends(admin)])
    ...
    with SessionLocal.begin() as db:
        ...
        for item in body.items:
            db.add(OrderLine(order_id=order.id, original_product_id=item.product_id,
                             ...
                             unit_price_paisa=catalog[item.product_id].price_paisa))
...
@app.post("/orders/{order_id}/audit", status_code=202, dependencies=[Depends(admin)])
...
@app.post("/orders/{order_id}/retry", status_code=202, dependencies=[Depends(admin)])
...
@app.get("/orders/{order_id}/outbox", dependencies=[Depends(admin)])
...
@app.get("/orders/{order_id}/audit-log", dependencies=[Depends(admin)])
